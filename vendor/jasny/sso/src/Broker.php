...
class Broker
{
     ...
     * @return string
     */
    public function getAttachUrl($params = [], String $attach = "attach")
    {
        $this->generateToken();
    ...
    public function getUserInfo()
    {
        ...
        if (isset(request()->outSession)) {
            if (isset(request()->isOut) || request()->outSession == 'true') {
                ...
                $params = ['return_url' => $returnUrl];
                $url = $this->getAttachUrl($params, 'logout');
                // dd(\Request::cookie(config('sso_token_12345')));
                header("Location: $url", true, 307);
                echo "You're redirected to <a href='$url'>$url</a>";
                exit();
            }
        ...
        if (!isset($this->userinfo)) {
            // if (!$this->request('GET', 'userInfo')) {
            //get auth from server
            $protocol = (env('APP_ENV') == "production") ? 'https://' : 'http://';
            $returnUrl = $protocol . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
            $this->redirectBack($returnUrl);
            $params = ['return_url' => $returnUrl];
            $url = $this->getAttachUrl($params, 'userInfo');
            // dd(\Request::cookie(config('sso_token_12345')));
            header("Location: $url", true, 307);
            echo "You're redirected to <a href='$url'>$url</a>";
            exit();
    ...
    public function redirectBack($returnUrl)
    {
        $newUrl = str_replace(request()->getSchemeAndHttpHost(), '', str_replace('/login', '', $returnUrl));
        if ($newUrl == '') {
            $newUrl = '/';
        }
        \session()->forget('newUrl');
        \session()->put('newUrl', $newUrl);
    }
}
