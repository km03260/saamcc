                ...
                (response) => {
                    if (response.ok) {
                        if (response._row) {
                            ...
                            if (response._row.line_color != undefined) {
                                tr.css('background-color', response._row.line_color);
                            }
                            $(`#tfoot_${tblist}_input`).change();
                            $('.load_foot').change();
                            if (response._row_p) {
                                load_row(response._row_p.id, response._row_p, response.list_p ?? '');
                            }
                            if (response._clicked) {
                                $(response._clicked).click();
                            }
                            setTimeout(() => {
...
function chargeInput(field, id, value, type, model, options = null) {
    ...
    switch (type) {
        ...
        case 'drop':
            var options_str = ``;
            var split_car = options.includes('#') ? '#' : '|';
            options.split(split_car).forEach((options) => {
                var val = options.includes(";") ? options.substr(0, options.indexOf(';')) : options;
                var op = options.includes(";") ? options.split(';')[1] : options;
                options_str += `<option value='${val}'${(value == op || value == val) ? 'selected' : ''} >${op}</option>`;
            })
            return '<span class="appendInputField">' +
                '<select name="' + field + '"  data-model="' + model + '" class="ui search fluid dropdown field_ ' + field + '" id="' + field + '' + id + '" style="padding:4px; font-size:11px !important; height:25px">' +
                options_str +
                '</select>' +
