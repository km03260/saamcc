function resetField(url, list, classField = "editField", date_new = null, field_new = null, success_action = null) {
    ...
    $('.' + classField + ' .field_').each(function () {
        ...
        if (val_old != valeur && change == true) {
            ...
            ajax_post(datas, `${url}?methode=savewhat`,
                (response) => {
                    if (response.ok) {
                        if (response._row) {
...
function chargeInput(field, id, value, type, model, options = null) {
    ...
    switch (type) {
        case 'number':
            return '<span class="appendInputField"><div class="ui fluid input mini"><input style="" data-model="' + model + '" name="' + field + '" id="msg' + field + '_popup" value="' + value + '" class="field_ form-control input-editLine ' + field + '" type="number" row="1" autofocus/></div> </span> <span style="" class=""></span>'
            break;
        case 'decimal':
            return '<div class="appendInputField"><div class="ui fluid input mini"><input style="" data-model="' + model + '" name="' + field + '" id="msg' + field + '_popup" value="' + value + '" class="field_ form-control input-editLine ' + field + '" type="number" row="1" step="0.01" autofocus/> <span style="" class=""></span> </div></div>'
            break;
        case 'textarea':
...
$(document).on('dblclick', '.editFieldLine', function (e) {
    ...
    if (self.find('input').length == 0) {
        ...
        var model = columnChange.data('model') + '/' + id + appends
...
$(document).on("keyup", ".editFieldLine .field_", function (event) {
    ...
    var appends = columnChange.attr('data-appends') != undefined ? '?' + columnChange.attr('data-appends') : '';
    var model = columnChange.attr('data-model') + appends
